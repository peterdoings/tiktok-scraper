...
export class TikTokScraper extends EventEmitter {
    ...
    private async getVideoMetadataFromHtml(): Promise<FeedItems> {
        ...
        try {
            const response = await this.request<string>(options);
            if (!response) {
                throw new Error(`http request failed: ${response}`);
            }
            ...
            if (response.includes('SIGI_STATE')) {
                ...
                const matchResults = response.match(/ItemModule" *: *({.*}), *"UserModule/);
                if (!matchResults) {
                    throw new Error('Could not parse raw video metadata');
                }
                const itemModule = JSON.parse(matchResults[1]);
                console.log(`videoProps: ${itemModule}`);
